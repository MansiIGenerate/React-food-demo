...
describe('Email Validation', () => {
  test('displays error message for empty email',  () => {
    <Registeruppage />;
    waitFor(()=>{ fireEvent.change(screen.getByPlaceholderText(/Enter email/i), { target: { value: '' } });
  ...
  test('displays error message for invalid email format',  () => {
    <Registeruppage />;
    waitFor(()=>{
      fireEvent.change(screen.getByPlaceholderText(/Enter email/i), { target: { value: 'invalidEmailFormat' } });
    fireEvent.click(screen.getByTestId('submitbtn'));
     waitFor(() => {
      expect(screen.getByText(/Please enter a valid email address/i)).toBeInTheDocument();
    });
  ...
  test('displays no error message for valid email',  () => {
        <Registeruppage />;
        waitFor(()=>{fireEvent.change(screen.getByPlaceholderText(/Enter email/i), { target: { value: 'valid@email.com' } });
        fireEvent.click(screen.getByTestId('submitbtn'));
        waitFor(() => {
  ...
  test('displays no error message for a default valid email',  () => {
    <Registeruppage />;
    waitFor(()=>{ 
    fireEvent.change(screen.getByPlaceholderText(/Enter email/i), { target: { value: 'mansi@getDefaultNormalizer.com' } });
    fireEvent.click(screen.getByTestId('submitbtn'));
     waitFor(() => {
      expect(screen.queryByText(/Please enter a valid email address/i)).toBeNull();
    });})
  ...
  describe('Password Validation', () => {
    ...
    waitFor(()=>{
      ...
      fireEvent.click(screen.getByTestId('submitbtn'));
      expect(screen.getByText(/Password is required and should be at least six characters long/i)).toBeInTheDocument();
    })
    ...
    test('displays no error message for valid password', () => {
      ...
      waitFor(()=>{
        fireEvent.change(screen.getByPlaceholderText(/Password/i));
        fireEvent.click(screen.getByTestId('submitbtn'));
        expect(screen.queryByText(/Password is required and should be at least six characters long/i)).toBeNull();
      })
